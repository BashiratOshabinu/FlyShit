...
export function Tops(
  ...
  const { setAllCategory, allCategory, tops, setTops } = useContext(AppContext);
  const [topsID,setTopsID] = useState("")

  const [outfits, setFilteredOutfits] = useState([]);

  useEffect(() => {
    const q = collection(db, "outfits");
    const filter = query(q, where("category", "==", "tops"));
