...
export function Skirts(
  ...
  const { setAllCategory, allCategory, skirt, setSkirt } = useContext(AppContext);
  const [skirtID,setSkirtID] = useState("")

  const [outfits, setFilteredOutfits] = useState([]);

  useEffect(() => {
    const q = collection(db, "outfits");
    const filter = query(q, where("category", "==", "skirt"));
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "black" }}>
      <View style={styles.container}>
        <FlatList
          ...
          keyExtractor={(item) => item.skirtID}
