...
export function Fjackets(
  ) {
  const navigation = useNavigation()
  const { setAllCategory, allCategory, Fjackets, setFjackets } = useContext(AppContext);
  const [fJacketID,setFJacketID] = useState("")
  ...
  useEffect(() => {
    ...
    const filter = query(q, where("category", "==", "Fjacket"));
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "black" }}>
      <View style={styles.container}>
        <FlatList
          ...
          keyExtractor={(item) => item.FjacketsID}
          horizontal={false} 
          numColumns={2} 
        />
      </View>
