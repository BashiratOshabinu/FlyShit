...
export function Newin(
  ...
  const { setAllCategory, allCategory, newin, setNewin } = useContext(AppContext);
  const [newinID,setNewinID] = useState("")

  const [outfits, setFilteredOutfits] = useState([]);

  useEffect(() => {
    const q = collection(db, "outfits");
    const filter = query(q, where("category", "==", "newin"));
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "black" }}>
      <View style={styles.container}>
        <FlatList
          ...
          keyExtractor={(item) => item.newinID}
